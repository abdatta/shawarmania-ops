...
**Now do the part that convinces people.** In the yellow strip, set **connectivity** to *Offline: network dropped*, and ring three more bills. The counter keeps taking money; the sync indicator counts up and escalates at five waiting. Set it back to *Online* and watch the queue drain, with the bill numbers appearing only as each bill lands — never before, because bill numbers are the server's. Nothing here is simulated but the network itself: aeroplane mode does the same thing, and the control exists because nobody opens DevTools in front of a prospect.
