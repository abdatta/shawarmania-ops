...
- **The full billing day is already coherent.** It includes direct Mark Paid tender capture, a Cash + UPI split, an order paid on handover, a Zomato order paid when its rider collected, a reasoned cancellation, one open order, one payment not sent yet, one command needing attention and a repeat customer found only after their complete phone is entered. The paid examples reuse the same bills counted by Cash and the owner console, so adding lifecycle context does not invent revenue.
...
A shift is already open. Ring a direct sale—tap tiles, enter either a customer name or phone, tap **Mark Paid**, then **Cash**, then confirm **Mark Paid**. The screen clears in the same tick; nothing was awaited, and Undo restores the complete composer during its guaranteed six-second window. For split tender, key `100`, tap Cash, tap UPI for the ₹39 remainder, then Mark Paid.

Ring another item, enter a customer name or phone and choose the primary **Order** action. It appears directly in the landscape Counter's compact **Open orders** rail—there is no separate one-slot latest-order card. Its complete preparation lines and amounts, optional customer and total lead the card; `Order #xyz` is only a small reference, today's timestamp is relative, and the current biller's name is not repeated. Edit with the pencil icon to load the order into the full composer, add an item or change its customer, then save and see any prior draft restored. Mark it paid, or cancel after a preset fills the editable reason field; the paid bill moves below the divider into **Bills this shift**, where all four method totals remain visible at zero and each bill expands to immutable details. Repeat with Zomato for a rider collection. Enter `9000000101` to see exact-phone autofill. Card and Other never appear as payment categories.

Switch to **Admin → Billing**. Filter the bills by revenue date and open one detail, void it with a reason, then follow the instruction to ring the corrected sale manually on the enrolled tablet. In Open orders, cancel the stranded order with a reason. Delivery shows the same problem as read-only metadata and never exposes its contents or customer.
