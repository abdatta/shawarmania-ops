...
**Days are piling up unapproved** → the owner's attendance surface shows the count per outlet and the oldest waiting date, so this is visible without opening each shop, and choosing an outlet from that list brings the view to it. Each day is then approved on its own: there is deliberately no bulk action, because a button that settles a whole morning is how an arrival nobody saw gets counted. The position is read once and reused for a minute, so a run of approvals is not a run of GPS reads. Approving from away from the outlet is allowed and asks for a reason, which is stored on each day and readable by the person it is about — so a week of approvals given from home is a week of visible reasons rather than an invisible habit.
