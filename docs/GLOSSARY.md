...
A shift opens through a two-device handshake — the tablet takes a username and shows a four-digit code, and the named person enters that code on their own phone — and it ends when that person ends it, when somebody else takes the counter, when the tablet is removed, or by itself at the outlet's cutover. A shift never spans two business dates.
