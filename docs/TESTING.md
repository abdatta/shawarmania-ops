...
`e2e-auth/overview.spec.ts` checks the same live layout against the local backend, including delayed monthly responses and manager outlet isolation. The Ledger timing REST suite (`npm run test:rls`, its own phase) delays every request by 250 ms and asserts that a Ledger day settles within one round trip (two round trips' time, with headroom) and a month within two, with a month's request count independent of its dates. A local stack otherwise hides the cost entirely: a month measured 300 ms locally and 16 s in production. It records an anchor and a count per outlet through `record_drawer_observation` so the measured day reads its balances, and removes them with the service-role key afterwards. It holds `ledger_month_inputs` equal to `toMonthDayInput(getDay(date))` for every date of the month, and compares Overview sales, revenue, expense totals and commission qualification against the Ledger reader. `supabase/tests/58_the_ledger_reads_in_two_round_trips.sql` pins both ledger read functions on the spec's worked example and asserts that every reader the drawer does not admit is refused, not answered empty.
...
npm run lint      # ESLint + no-hex + backlog/spec indexes + Edge Function config + agent instructions
...
**`npm run lint` gates the agent instruction files against a toolchain that is not here.** This repo drives OpenSpec through plain files — fixed folders under `openspec/`, a `spec-driven` `config.yaml`, and `npm run roadmap:sync`. There is no `openspec` CLI. The workflow files were vendored from upstream, where that binary exists, so for months every `/opsx:*` run spent a few failed calls discovering its absence and then improvised a fallback, differently each time; the wasted tokens were the visible symptom and the shape-drift in the change folders was the real cost. The check fails, naming file and line, on any `openspec <subcommand>` invocation under `.claude/` or `.agents/` — describing the CLI stays permitted, and the stripped files do exactly that in a table mapping what it would have returned to the constant it is here. It fails equally on the second drift: Claude reads `.claude/commands/opsx/` and Codex reads `.agents/skills/openspec-*`, nothing syncs them, so a workflow present in one tree and absent from the other is named. Contents are deliberately not compared — the trees are written for different hosts and their prose legitimately differs. **This runs in the `Prose` tier as well as in full lint**, for the same reason the backlog check does: editing an instruction file is itself a prose-only commit, so gating it only in the full suite would disarm it for exactly the commits that can reintroduce the calls.
...
The reason is a gap #51 would otherwise have shipped with. Both existing projects run at widths where navigation is the left rail, whose sections are open by default — so every e2e spec in the repo exercises the rail and none had ever drawn the bottom bar. The two-level phone bar is the thing that change was actually asked for: a card anchored to the tab that opened it, a group that opens from the address and toggles from any tab, and a sum that becomes its parts when the group opens. Unit tests cover all of it in jsdom, where both rows are in the document at once and neither is laid out — which is precisely where a geometry bug lives.
