...
- **Location is captured only in direct response to a relevant action** — each employee check-in attempt, an approval or correction to present, an outlet position capture, and opening one of the Cash drawer's recording sheets (a count, a collection or a spend), where the reading decides whether the recorder is asked why they were elsewhere. A retry is a deliberate additional reading and its immutable evidence is retained; denial, correction to absent and retry-policy correction read no location. There is no background tracking, no continuous location, and nothing that reports where someone is between those events. Do not add one without a deliberate decision recorded in a change proposal. This is enforceable by reading one file: `src/lib/geolocation.ts` is the only module in the app that touches `navigator.geolocation`, and tests assert that neither an employee's home screen nor a manager's attendance day reads a position while it merely sits open. The Cash drawer reads one only when a sheet is opened to record something, never while the surface is merely on screen, and never as a watch.
- **One reading per action, and never one carried over.** An approval reads the position in direct response to the action that records it, whether that action settles one day or a selected set. There was a sixty-second window in which a reading could stand in for a fresh one; it existed only so that approving one person at a time did not cost a GPS read each, which deciding a set solves properly, and while it stood a stored approval position could be a minute old. It is retired, so every stored approval position is a reading taken for the act it is recorded against. One reading covering a set spanning two outlets is measured against each outlet's own fixed point — arithmetic on one truthful statement about where the manager stood, not a claim to have been at both.
...
- **The employee can see everything recorded about them.** Their own attendance history shows every attempt and decision, with the same coordinates, distances, accuracy, late tags, denial/correction reasons and approvals available to the owner — including the approver's name and whether that approver was at the outlet. Names are snapshot onto history precisely so the person it concerns can read them. Every view renders through the same components, so they cannot drift apart. Asymmetric visibility in a monitoring feature is how it becomes something people resent.
...
- **A phone clock is neither attendance evidence nor authority.** A phone self-check-in stores the database receipt time and derives its business date there, from the outlet cutover. The GPS reading timestamp is not retained as a second employee-location fact, so a mis-set device cannot block, backdate or future-date an arrival. Manual entry and time correction deliberately retain the manager's attested time instead.
- **Historical manual entry widens time, not authority or monitoring.** The database accepts a manager-attested arrival only when the caller currently manages the outlet, the subject is current Employee/Biller staff there, the subject's assignment covered the named date, and the past instant resolves through that outlet's cutover to that date. The database stamps the caller; no client-supplied enterer is trusted, and the record contains no coordinates.
- **The current-time read stays within the same tenancy boundary.** `attendance_current_context` is a security-invoker read over the outlets the caller can already select. It returns a shared server instant and each readable outlet's current date; asking for another outlet merely omits it, rather than disclosing its clock or cutover.

- **A retry widens neither authority nor visibility.** It is another immutable
  reading on the same person/business date, allowed only at a live outlet where
  that employee is assigned and only while that outlet still calls the explicit
  date current. A former-outlet manager retains their own local attempt and
  decision but learns no outlet, time, position, outcome or manager fact from a
  later attempt outside their scope. The subject and Super Admin retain the full
  sequence. A global prevent-retry decision is enforced at the canonical day,
  not by hiding buttons in one outlet's UI.
...
- **Tablet removal is immediate and permanent**, enforced inside every policy rather than by waiting for a token to expire. It ends that tablet's live shift and cancels its pending request in the same transaction, and touches no other tablet at the outlet, no other shift and no human assignment. There is no paused state: a paused tablet is a security question that a removed one is not.
- **A redeemed setup code is not yet a counter.** Redemption and the browser establishing its session cannot share a transaction, so the row redemption writes reaches nothing until the browser proves it holds a session, and lapses on its own when the code's own expiry passes. The predicate every policy asks is therefore `removed_at is null and session_proven_at is not null`. A lost response between the two acts costs a code and not a till, and leaves nothing for an admin to clear.
- **A tablet reads its own outlet, and only while somebody is on it.** With several tablets at one outlet, a counter holding a live shift reads that outlet's orders and its sibling tablets' labels, because one kitchen serves both tills and a refusal has to be explainable before it is met. Both reads stop at the outlet, are asserted by hand-crafted request in both directions, and are gated on the shift rather than on being set up: a tablet standing idle overnight still reaches nothing. Bills are **not** widened — `Bills this shift` stays this shift's, and a till reads no money history but its own.
