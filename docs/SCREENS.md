...
**A screen that can read several outlets at once lets you select several** (#29). Attendance is the first: its selector is a toggle per outlet rather than a dropdown, because a native multiple-select needs ctrl-click and is close to unusable on a phone. The result is one combined list with each entry naming its outlet, never one list per shop. **The last selected outlet cannot be cleared** — an empty selection is a blank screen asking a question nobody asked — so that toggle is disabled rather than swallowing the press. Somebody who may see one outlet is still offered no selector at all. Selecting several widens nobody's reach: the database returns only what their assignments allow, and the selection is intersected with that.
...
**The owner sees a chip per outlet.** Above the day, a Super Admin gets one chip for each outlet holding unsettled days, each carrying its own count; the one already in scope says so, and choosing any other brings the view to it, so noticing and acting are one gesture. The row appears only while some **other** outlet is holding work — when the outlet in scope is the only one, it would repeat the header and the day controls while pointing nowhere, so it is not drawn. That count spans every business day, which is why it is deliberately not the same number as the day's own badge: a shop can hold nothing today and a week behind it. This replaced a paragraph headed "Days waiting for a manager" that printed each outlet's oldest waiting date — the date was not lost, it changed job, and now marks the earlier-days arrow instead.
...
**By staff** is one person over a range of dates, defaulting to this month, with the counts: present, late, absent and waiting. It exists so somebody can work out pay by hand, which is why **each business date counts once** whatever shop it was worked at. It offers **staff only** — a range of days for somebody whose days are not tracked would be a pattern of nothing, so a non-staff person's record is read on the day it belongs to, which is where anybody settling one is already standing. The arrows move a month at a time and the two date fields make it any range at all.
