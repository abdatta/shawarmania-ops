...
**Cash drawer** — the surface this business was commissioned to get right, at the second attempt. **It opens on a balance, not a date picker**, because that is the question the collector has when they walk in: what should be in the drawer right now. The running balance, the last count, what was left, and the cash bills and cash expenses since — each with its own count of contributing rows.
...
**Count the drawer** is the primary action and takes three inputs: when it was counted, what was in it, and how much is being collected. Four taps on an ordinary night.
...
**A count time that differs from the entry time is approximate by default**, with a ±15 minute window and one control to assert certainty. The surface then states, in rupees, how much cash moved inside that window, so a ₹50 gap against ₹914 of nearby cash reads differently from a ₹3,100 gap against the same ₹914.
...
**Collect cash** takes an amount and an instant and nothing else: no reason, because collection is the routine act, and no actor picker, because the actor is the session. It says on its face that nothing is being verified.
...
**Record a cash spend** is a small link well away from the primary action, requires a reason, and states on its face that it will **not** enter the month's operating expenses. Drawer cash that buys a ₹40,000 fridge has to move the drawer without wrecking a cash-basis operating figure, and that is the only reason this escape hatch exists.
