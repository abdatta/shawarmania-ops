...
The **manual Ledger form keeps working at its own route and simply leaves the navigation** while the derived statement is proved. The fallback is a tab, not a runtime toggle, so both readings can be open at once and one business date compared in each.
