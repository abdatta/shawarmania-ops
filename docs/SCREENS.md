...
>
> **The counter and the manager's operational surfaces are built and demo-gated** (#6, #7): the billing counter with its shift screens and sync indicator, and Menu, Stock, Expenses and Daily cash — all walkable at `/demo/*` against mocked data, none of them connected to real data yet. Billing became real in #10. **Daily cash was not made real: it was replaced.** `cash-is-counted-not-closed` (#11) removed day close and built a Cash drawer and a derived Ledger in its place, because the drawer is counted mid-shift rather than at the end of a day. The old screen is `hidden`, not deleted — that change drops and renames nothing, so a revert is one registry edit. **Stock is not made real either**, and is shelved (`openspec/todos/inventory-is-shelved.md`). Expenses are already live through #36 and #38.
>
...
**Billing history** — a phone-first outlet record for Franchise Admin and Super Admin. It asks two questions and no more — **which outlet, and which business day**. The outlet is chosen with the same remembered chips every outlet-scoped surface uses, in the page header; the day is chosen in the same bar the Ledger uses, a step either side of the date itself, and forward stepping stops at that outlet's current business date. The bill-status and payment-method pickers that once sat beside them are gone: the list is already one outlet's one day, and every row names its own state and tender, so they re-stated what was on screen. The day defaults to the outlet's current business day and reads **Today**. Tapping it opens the platform calendar; a past choice reads in the app's format, for example `12 Aug 2026`, rather than an ambiguous browser placeholder. Every collapsed row is a scannable Paid/Cancelled summary with amount, tender, an outlet-local Today/Yesterday time where applicable and the biller name; selecting it expands that bill directly beneath the row, with only one open at once. Expansion uses the ordinary surface border, not an accent outline, and a short height/opacity transition; changing to a lower bill holds the tapped summary in place while the prior detail closes. The order items are followed by a green outlined **Paid by** payment banner and right-aligned total; when a bill is cancelled, its equivalent red, attributed cancellation banner appears before the items and the retained paid banner uses a quieter green outline. **Customer details** and **Bill timeline** are separate two-column nested disclosures, closed by default so optional customer and audit facts do not consume the phone viewport. Missing optional customer facts are said rather than silently omitted, while customer phone remains out of summaries and diagnostics. Cancellation is an exceptional action: **Cancel this bill** opens a neutral dialog that requires a reason and final confirmation; the original remains visible as a compact, attributed cancellation row, calling the signed-in actor **You**, with no duplicate success message. **Open orders** shows each stranded order's captured items, total, customer details, creator and time before showing a single **Cancel this order** action; that action opens the same neutral, reason-required dialog pattern, with editable common reasons and a concise final confirmation. **Sync status** replaces the former receipt-by-receipt Delivery feed: it says whether recent tablet activity has a problem, groups routine successful activity by business action, and reveals non-identifying short references/result metadata only inside **Show technical details** — never customer details, command contents or tablet correction controls. A problem names the order it concerned where the refusal named one, and where a resend cannot help it says so, so the manager sends the tablet towards discard rather than towards a correction that would add another row. The tab itself carries the count in the page's own parentheses, `Status (1)`, so a refusal is not invisible behind a tab that looks quiet; it is not a badge, because a manager cannot clear a refusal from this screen.

**The day's four figures** — Cash, UPI, Total and **AOV** (average order value) for settled bills at the selected outlet and business day, in one row **above the tabs rather than inside one**. The surface is opened to find out what the day took, and behind a view named after tablet sync that answer was two taps away; here it is on screen with the Bills list the surface lands on, and stays there through Open orders and Status. Cancelled bills count towards none of them, Total is the sum of the two cards beside it rather than a separately derived number, and AOV is that total over the paid bills of the same day. Each figure keeps the app's ordinary display size and shrinks only when its own card cannot hold it — never below the label above it — so a four-across row on a phone reads with the same even padding as two-across on a tablet. The counter's own **My shift** rail keeps its two-column Cash and UPI cards: its scope is one shift, not a day.
...
**Cash drawer** — the surface this business was commissioned to get right, at the second attempt. **It opens on a balance, not a date picker**, because that is the question the collector has when they walk in: what should be in the drawer right now. The running total sits at the right end of its own label's line with the chips that qualify it directly beneath — when it was last counted, how many days are uncounted, how many tablets are behind. Under those, three figures named for what they are and signed rather than tinted: **Last Left**, **Cash from Bills** with a leading `+`, and **Cash Expenses** negative and in the danger colour, each with its own count of contributing rows.

The old Daily cash screen is **`hidden` rather than made real**, and this is why. The drawer is counted mid-shift, at a time the collector picks, sometimes after skipping a day or two, and sometimes entered an hour later from somewhere else. A count taken at 22:00 measured against a whole business date's cash sales produces a difference that is fiction — **₹4,640 of it in one month across two outlets**, measured on August 2026 production data. At Kanchrapara that is the ordinary case rather than an edge: 8 of its 13 cash dates traded past 22:00.

**Count & Collect** is the primary action and takes three inputs: when it was counted, what was in it, and how much is being collected. The count and the collection are one physical act, so they are one control. The collection field starts at `0`, so an ordinary night — count it, take nothing, save — is three taps and one number.

**The difference appears the moment the counted amount is typed**, before anything is submitted — with its direction in words as well as by sign, because a minus is the first thing a small screen loses and *"₹240 short"* is not a sentence anyone misreads.

**Every count time is approximate**, with a ±15 minute window, and **nothing on the surface offers to assert an exact instant**. That reverses the earlier rule that only a recalled time was approximate, on the owner's own reasoning: counting a drawer takes a minute or two with a queue in front of it, the counter keeps trading while it happens, and a bill rung during the count lands on one side of a boundary nobody was watching. The instant a person supplies is the middle of an act, never its edge — whichever of **Now**, **15 min ago**, **30 min ago** or an explicit date and time they picked. The surface states, in rupees, how much cash moved inside that window, so a ₹50 gap against ₹914 of nearby cash reads differently from a ₹3,100 gap against the same ₹914. The window is the same width for every option: it models the duration of the count and the trade during it, which does not shrink because somebody pressed the button sooner.

**An exact coincidence is reported as a fact; a nearby time is never suggested.** This is the most important refusal on the surface. Moving the count boundary does not move the expected figure smoothly — it jumps one bill at a time, so the reachable values are a small discrete set. An *exact* hit is unlikely to be chance, so saying *"₹854 short — that is exactly the 3 cash bills rung between 22:04 and 22:12"* is an observation. A *near* hit is almost always available: a genuine ₹500 shortfall matches nothing, but the nearest reachable boundary would shrink it to ₹222, and ₹222 reads as rounding noise where ₹500 reads as a missing note. The denser the trade the denser that set, so the ability to explain anything away is strongest on exactly the nights when a loss is easiest to hide. The surface therefore offers a movable boundary over the nearby cash bills — so the recorder corrects the time **from evidence they recognise** — and lets the difference be whatever that produces.

**Only Collect** takes an amount and an instant and nothing else: no reason, because collection is the routine act, and no actor picker, because the actor is the session. It says on its face that nothing is being verified.

**A negative amount is cash added to the drawer**, through the same field, with no second control. The scenario is real and already in production: a day with weak sales or heavy cash expenses leaves too little in the till, so the collector puts some of their own back at the moment of counting. **The surface says what a minus means on the keystroke that creates it**, not on submit — the stated action, the balance preview and the confirming control all flip with the sign. That alert is the entire protection against a mistyped minus.

**Other Spend** sits in the quieter row well away from the primary action, requires a reason, and states on its face that it will **not** enter the month's operating expenses. It is a control that looks like one: it used to be rendered as bare text with no boundary, which is distance from the primary action bought at the price of nobody recognising it. Drawer cash that buys a ₹40,000 fridge has to move the drawer without wrecking a cash-basis operating figure, and that is the only reason this escape hatch exists.

**Where the recorder stood is read, not typed.** Each of the three recording sheets takes one position when it opens, through the same module and the same fence rule attendance uses, and sends it with the write so the database derives the distance itself. Inside the fence the sheet says *at the outlet* and asks nothing. Outside it — or with no fix at all, which is the same claim as far as the record is concerned — it names the distance or why it could not tell, and **requires a reason before it will save**. Nothing is refused for being elsewhere; the record just says where you were. There is no watch and nothing samples in the background.

**Past counts are a paged list of disclosures.** A closed row is one line: the instant, the verdict beside it — `matched`, `₹200 short`, `₹40 over`, `first count` — then the amount and the chevron at the right edge, plus a broken opening if there is one. Where all of that cannot fit, the chips wrap and the money does not move; nothing on the row is ever clipped. Who recorded it, what they took, why they were away, its adjustments and the control to adjust it are all behind the row. Rendered together they were five lines a count and a wall by the fourth. The list pages rather than stopping at a cap: older counts arrive as the end of it comes into view, a button does the same for a reader scrolling by keyboard, and the list says when it has reached the oldest count.

**Adjust a count** appears on any observation a later one has anchored on. Before that lock the same row simply offers an edit, with no reason and no trail — freezing on save would make a typo permanent and push people to record a compensating adjustment for a mistake nobody has relied on yet.

**Work that arrives after a count is reported beside it, never inside it.** A cash bill whose payment instant falls in an already-observed interval raises a *reconciliation exception* naming the bill, its amount, when it was rung, when it landed, and what the difference would have been. Resolution is an attributed acknowledgement or a fresh count; neither alters a stored figure. A late arrival may also **explain** a recorded variance — an over that turns out to be an unsynced tablet's cash — and then the recorded figure stays with the explanation beside it.

**An unsynced tablet advises and never blocks.** The collector is standing at the counter holding cash; a count refused because a device is behind is a count that does not get recorded at all.

**Ledger** — the per-day reading, **derived on read with nothing typed into it**. Revenue with Cash and UPI from counter allocations and each aggregator's gross, commission and net carrying its settlement state — a commission not yet stated reads *not known yet*, never nought, and the month is presented as a ceiling while any day is in that state.

The drawer section is **ordered by instant rather than grouped by category**, which is what makes a collection legible: an expense at 18:10 sits above the 22:00 count and one at 23:00 sits below it. An unexplained variance takes its own line inside the count block, so the block still adds up.

**The float left and the closing balance are named separately and never with one word.** On the worked example they are ₹1,450 and ₹3,504, and the reading says plainly that what was left is not the next day's opening.
...
The **manual Ledger form keeps working and simply stops being the entry called `Ledger`** while the derived statement is proved. It keeps a navigation entry of its own, **Notebook**, directly after it. The fallback is a tab, not a runtime toggle — and a tab you have to type a URL to reach is not one — so both readings can be open at once and one business date compared in each. The derived statement owns the `ledger` route and the notebook sits under it at `ledger/notebook`, which is what makes the navigation's nesting true: Expenses, Zomato, Swiggy and the notebook are all readings inside one group rather than children of a form being retired.
...
**Ledger** — *temporary (#36). `cash-is-counted-not-closed` (#11) takes the `Ledger` name and route with a derived statement that has no input fields, leaving this form live at `ledger/notebook` with its own **Notebook** entry as the fallback and as the reader of the trading period before each tablet existed; `retire-the-manual-ledger` (#12) then carries those rows across and removes the surface.* Where the owner records expenses and drawer facts while sourced aggregator trade arrives separately. Each outlet has an explicit future-only **Counter billing starts on** date on the Outlets screen. Before it, Cash and UPI remain typed exactly as before. From it onward those two inputs disappear and the values are labelled **from counter**, derived once from settled bill allocations. **Zomato and Swiggy are both readings, never inputs.** Swiggy's provisional stated gross comes from timestamped Finance order detail and settles from the payout annexure; the source chip makes its provisional/final state legible. Two views sit behind one toggle, one outlet at a time through the same switcher every other outlet-scoped screen uses. It sits **ahead of People** in the navigation, because it is opened every night and People is opened when somebody joins or leaves.
...
The entry card is laid out for the device it is used on. Before an outlet's billing handover, money fields sit **two to a row** with the unit inside the box, so `Cash` and `UPI` read as words instead of `Cash (₹)` down a column of full-width inputs. From the handover date those two boxes become read-only **from counter** values. Zomato and Swiggy are **outlined readings, not entries** — gross, commission, net and a chip naming their source/state. A commission not yet stated reads "not known yet" rather than nought. The rules the ledger runs on are behind a marked control beside each section title rather than printed under the fields: they are read once and then known, while the form is opened nightly, and left on screen they cost more height than the fields do. Labels are short and the hidden half of each one carries the rest, so `As stated` announces itself as "As stated for Swiggy, in rupees" — an outlined block means nothing to somebody who cannot see it.
...
**The month** shows revenue by all four channels with each aggregator **netted by the commission recorded against each day** — so a week whose commission moved is right on both sides of the change — and a month containing a day whose commission is not yet known is a **ceiling** that says so. Expenses are grouped by their normalised stored category with every line and its optional note behind the total, and an estimated profit states in words that it is a **cash-basis operating estimate**. Operating, and it says why: capital spending is deliberately not recorded here, so the figure answers whether trading covered running costs rather than where every rupee went. A month nobody has written in says so instead of showing `₹0`, because zero is a measurement and an empty month is not one.
...
**Zomato** *(owner-only, #42/#43/#44)* — reached from the ledger, it shows what the sync has done and the few things needing the owner: a header line saying when it last ran and whether it succeeded, then a list where a row is an event rather than a run, so the twice-daily runs that changed nothing add no clutter. Rows needing action open with their actions — a disputed week offers **Re-check** or **Accept the difference** on the record, never a silent write; a lapsed session offers **Reconnect**; a possible-duplicate expense shows both rows in full and can be marked two real purchases. **Read now** is offered only when reading again could say something new. Beside the header, a thinner **Hyperpure** line carries its channel's state in the same vocabulary — All quiet, Reading, Session ended, Stuck — and its own **Reconnect**: reconnect is a repair ladder now, so with Zomato still signed in a lapsed Hyperpure repairs itself silently and no code is ever asked for; only a genuinely dead Zomato login leads to the shared code card, which appears when the code actually arrives and not before, stays on screen however often the page is left and revisited while the login waits, and a reconnect that signs Zomato in but leaves Hyperpure behind says so on this line rather than passing as a success. Beneath both, **Upload a statement** is the fallback for when the automation is blocked: a quiet control naming the three files it takes — a Zomato order history, a Zomato settlement, a Hyperpure statement — read by the same parser the robot uses, reporting back per outlet, refusing a file it cannot place in the file's own words.

**Swiggy** *(owner-only, #47)* — a separate ledger tab with its own session, reader and event history; it contains no Hyperpure line or action. **Read now** starts the dedicated Swiggy writer for the selected outlet's enabled Swiggy references only, so a request cannot read another outlet or silently become a read-only run. A configured outlet with no captured Swiggy session offers **Reconnect** immediately, even before a scheduled reader has recorded a lapse. That reconnect probes and, when necessary, starts only the Swiggy login/OTP path; an outlet with no enabled Swiggy reference remains **Not connected** and has no fabricated action.
...
**Home** — one large check-in button, today's status, and the outlet they are assigned to. “Today” is supplied by the database for every assigned outlet, using that outlet's cutover, rather than inferred from the phone clock or the GPS reading. The screen refreshes that context when it returns to the foreground and after a named-day refusal. A recorded arrival says plainly that it is **waiting for a manager to approve it**, never that the day is done: it counts for nothing until somebody vouches for it, and a screen implying otherwise would be the misunderstanding this design exists to remove. An arrival after the outlet's deadline reads as late.
