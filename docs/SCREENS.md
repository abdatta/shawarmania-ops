...
- **Three columns, at every width, and nothing folds away.** The current bill and the activity column are the same width as each other; spare width goes to the menu, which is the column that reads better wide. Below the width three columns need, **the workspace scrolls sideways** — it does not rearrange, and no column becomes a tab, a route or a disclosure. A counter that reflows is a counter whose controls move while somebody is reaching for them, and each column is about a phone's width, so the narrow case ends up as three swipeable panels rather than as a compromise. Only the workspace scrolls horizontally; the page never does. Menu tiles are laid out against their own column's width, not the viewport's.
