...
**The outlet in the selector is remembered.** One choice per signed-in person, shared by every outlet-scoped screen and surviving a reload, so an owner who moves between Attendance, Expenses, Stock and Cash answers the question once rather than on each. It is checked before it is used: an outlet they may no longer see is replaced by the default rather than shown, and signing out forgets it, because these are phones that get handed over. It remains a filter that confers nothing — the database decides every write from the assignment.
