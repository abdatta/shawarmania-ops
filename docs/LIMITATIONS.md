...
### One customer identity, a gold label, and deliberately little else

Customers are business-wide since `global-customer-identity` (#32): one normalized phone is one person at either outlet. **Gold membership is a label on that identity** (`a-gold-member-is-a-label`, #57): granted by hand, business-wide, and visible at the counter as a star so a biller can decide to do something about it with the discount controls that already exist. It is deliberately **only** a label — no automatic discount, no points, no automatic appointment, no tiers beyond gold. The owner, and a manager over their own outlets' customers, read thirty days of visits and spend on a card, summed from bills when asked; no counter sees any figure. There is still no cross-outlet history, no marketing and no export. See [`openspec/todos/customer-loyalty-and-cross-outlet-insights.md`](../openspec/todos/customer-loyalty-and-cross-outlet-insights.md) for what waits.
...
- **A name can be corrected, and nothing else about a customer can be changed.** The owner corrects any name from the Customers card, and a manager one only their outlets have ever served (#57); a correction changes the profile and never a bill or order, which keep the name they snapshotted. A name cannot be erased, and no screen deletes, merges or re-numbers a customer. **The counter's one chance to give a name is the moment it first saves a number**, which is why the UI insists on one there.
...
- **A partial number reaches this outlet's customers only.** Four digits or more suggests one person this counter has already served, with the others counted and never listed. The directory stays unbrowsable **from a counter**: there is no prefix, wildcard, list or count verb over `customers` for a till, and the business-wide lookup still takes a complete number. The owner and a manager do search and list it (#57) — the manager only over their own outlets' customers — through functions no counter role can call. The scope is what makes the shorter prefix safe — a prefix over the directory would read one franchise's customers from another's till.
