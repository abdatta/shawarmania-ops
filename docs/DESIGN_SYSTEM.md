...
- **`--primary` and `--success` are gated against `--surface-raised` at 3:1.** They identify the two state controls on a pipeline card while their boxes are unchecked, drawn straight onto the raised card, so the tone itself has to clear the non-text threshold rather than relying on the pair it ships with. Both do, in both themes. The checked state draws its mark in the fill's own foreground token, which the `--on-primary` / `--on-success` rows already gate.
...
- **A count the reader cannot clear is not a badge.** Where a number is worth surfacing but the person reading it has no action to take on it — a diagnostic tally, a figure that ages out on its own — it goes in the parentheses the surface already uses for its siblings (`Bills (24)`, `Status (1)`), not in a `Badge`. Reaching for the badge there would make it a status light, which is the one thing the rule above forbids.
- **A checkbox in a button states a fact; it does not name the next press.** The pipeline card's two controls (`src/features/billing/state-toggle.tsx`) are toggle **buttons** carrying `aria-pressed`, whose visual language is a checkbox and whose labels — `Prepared`, `Paid` — never change. A control that told you the verb for its next press has to change its word every time it is pressed, and the churn that causes is what the control exists to remove.
  - **Two token pairs, one per tone.** Unchecked is the ordinary secondary treatment with the box outlined in the tone's own colour: `--primary` for preparation, `--success` for payment. Checked fills the whole control with that colour and the box **inverts**: a solid square of that control's own foreground token — `--on-primary`, `--on-success` — with the tick cut out of it in the tone. White box, ember tick. Never a literal grey, which would be muddy against the dark theme's bright flame orange and weak against the light theme's ember; using the pair the fill already ships with means both halves of the mark inherit a ratio the validator gates, the tone-on-foreground direction being the same ratio as the label-on-fill row read the other way round.
  - **The box is sized against its label, not against the control.** 17px beside `text-base` — which is 14px at this app's 14px root — so the square is about 1.2× the type it sits next to. It was first drawn beside `text-sm`, at 1.4×, and the box read as the loud part of a control whose label is the part that means something. If either the box or the label moves, the other moves with it.
  - **Solid, not outlined, once it is ticked.** An outlined box with a tick the same colour as its border is a smaller difference than the state deserves — from a step back it is one shape either way. A filled square reads as ticked across the counter, and putting the tone in the tick rather than the border keeps the box saying *which* of the two facts this control is about as well as *whether* it is recorded.
  - **Checked and unchecked differ in shape**, an empty square against a ticked one, which is the "colour is never the only signal" rule satisfied by construction rather than by a second badge beside it.
- **A fact that cannot be changed is drawn without control chrome, never as a disabled control.** Where the reader genuinely may not act — another till's order on the shared pipeline — the state is printed on the card: no border, no press affordance, not a `button`, and no `aria-pressed` to misreport a state nobody can toggle. A dimmed button is a promise the screen is refusing to keep, and billers read it as breakage. This is distinct from the disabled-submit rule above, which is about a form the reader *can* complete.
