...
- **Waiting for a read reserves the space** (`src/components/ui/loading.tsx`). `LoadingList` stands in for a list of cards and `LoadingBlock` for a strip; both read semantic tokens only. The rule they exist for is that **content arriving must not shift what is already on screen** — a line of "Loading…" is one line tall and a list of cards is not, so every read used to end with the controls above it jumping. Before #29 there was no loading component at all and each screen wrote its own sentence, which is also how a surface ended up showing one outlet's rows under another outlet's name. The region is `aria-busy` and carries a name saying **what** is loading, so a reader who cannot see the shimmer is told the same thing; the animation is Tailwind's `animate-pulse`, which stops under `prefers-reduced-motion` while the reserved blocks stay, so the waiting state never depends on motion alone. Use it whenever the data on screen no longer matches what is being asked for — a filter or scope change, not only a first load.
