...
| [The Specs README Index Has Drifted](./specs-readme-index-has-drifted.md) | Documentation gap | Open | Process | The next change that archives a capability, or any change touching openspec tooling |
| [The Agent Contract Describes Built Features As Unbuilt](./agent-contract-describes-built-features-as-unbuilt.md) | Documentation gap | Open | Process | The next change that touches `AGENTS.md`, or the next `*-live` change |
| [An Unknown Command Argument May Vanish On The Wire](./undefined-command-arguments-vanish-on-the-wire.md) | Verification gap | Open | Data access | Before the next `*-live` change adds a command family, or on any report of an action that fails while writing nothing |
