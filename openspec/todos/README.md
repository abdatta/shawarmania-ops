...
| [Expense Payment Method Inherits The Bill Enum](./expense-payment-method-inherits-the-bill-enum.md) | Design gap | Open | Expenses | **Re-pointed 26 Aug 2026**: `retire-the-manual-ledger` (#12), which is already renaming the real expense table and regenerating its types |
| [Raw Materials Is Identified By A Word Nobody Types](./raw-materials-is-identified-by-a-word-nobody-types.md) | Design gap | **Dissolved** | Reporting | Nothing to do. Inventory is shelved and #12 withdraws the consumption basis, so the matcher has nothing to feed. Returns only with inventory |
...
| [A Near-Miss Category Should Be Caught In Expenses Too](./near-miss-category-matching-reaches-expenses.md) | Feature | Open | Expenses | **Re-pointed 26 Aug 2026**: the first duplicate spelling that actually costs somebody something. Deliberately not bundled with #12's rename |
| [The Ledger Handover Still Has To Be Done, Outlet By Outlet](./ledger-handover-per-outlet.md) | Operational rollout | **Dissolving** | Billing / Ledger | **No longer owed.** `billing_live_from` gates only the manual form's typed Cash and UPI; the derived statement reads bills without it. Closes when `retire-the-manual-ledger` (#12) drops the column |
