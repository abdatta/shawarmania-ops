...
| [A Franchise Admin Cannot Read The Channel Mapping](./franchise-admin-cannot-read-channel-mapping.md) | Defect | Open | Aggregator sync | A manager asks why their month names a channel the outlet does not sell on, or the next change touching aggregator policies opens the file anyway |
...
| [The Demo Month Reads As A Loss](./the-demo-month-reads-as-a-loss.md) | Fixture gap | **Reopened 2026-09-01** | Demo mode / Reporting | #52 restored the P&L, so the figure exists again — the demo August reads −₹20,564.15 because its expenses cover a notebook month while its bills cover a few days. Reachable by anybody walking the demo |
