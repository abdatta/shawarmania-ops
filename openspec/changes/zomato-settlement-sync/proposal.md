...
**Database.** Commission stops being a rate anywhere [owner, 2026-08-17]. `zomato_commission_bp` and `swiggy_commission_bp` are converted to exact-paise amounts and dropped, and existing production rows are rewritten by the identical rounding the application has always applied, so no historical figure moves by a paisa. A synced day and a typed day then hold the same two figures in the same two columns, with a settlement state as the only marker of which source wrote them; net is computed rather than stored, since a third figure could disagree with the two it came from. The row also gains the superseded typed figures with the moment they were superseded, and the provisional figures a settled day replaced. `manual_ledger_expenses` gains an external-source identity so a Zomato-sourced deduction is recognisable, deduplicated across runs, and not double-counted against a hand-entered one. A new record holds what belongs to no day: cycle-level deductions such as tax, and the unexplained difference on any week the owner accepts. Every new table and column ships its Row-Level Security policy and its isolation test case.
