...
> **Sequencing.** #56 `a-customer-is-a-phone-number` must be archived first. It
> builds the composer's customer control and the dialog that this change draws
> the member mark on, and it leaves the mark's slot deliberately empty. Starting
> before it lands means drawing on a surface that is still moving.
...
> production on 2026-09-18 held 1840 bills, zero with a phone, against an empty
> `customers` table, so `#56` deliberately backfills nothing. Every card reads
> empty until the new flow has been running a month — expected, not a defect, but
> say so before the demo rather than during it.
...
- [ ] 1.1 Extend the mock customers adapter with membership: grant, revoke, the derived current state, and the thirty-day figures. It must enforce the real boundary the way `mock/customers.ts` already does — only the owner grants, only a billing context reads the mark, and neither can reach the other's path.
- [ ] 1.2 The owner's customer surface: search by complete phone, plus a recent list requiring no search. Registered in `src/gates/registry.ts` as a Super Admin surface, **Setup group, labelled `Customers`, ordered after People**.
- [ ] 1.3 The customer card as a **modal, never a route** — no address identifies a customer. Identity block, then the derived block, in the shape recorded in `design.md`.
- [ ] 1.4 Rename in place: the pencil turns the name into an input and itself into a tick. Icon-only, no button at the foot of the card.
- [ ] 1.5 Membership as a status row with one icon-only control beside it — a star to grant, a crossed star to revoke, because **the icon carries the verb and the text carries the state**. `Gold member since <date>` or `Not a gold member`.
- [ ] 1.6 Confirmation on both directions through the existing `ConfirmDialog`, identically. It opens over the card; `Modal` already handles the stacking and its `onClose` already stops propagation, so this is reuse rather than new ground.
- [ ] 1.7 The member mark at the counter: on #56's composer control, on the resolved match **inside** the dialog before it is accepted, on the open-order card, on the pipeline card and on the shift bill list. No date, no actor, no history, no figure, anywhere.
- [ ] 1.8 Demo fixtures: a member, a non-member, a member with a revoked-then-regranted history, and a customer whose bills are all older than thirty days — so the empty-window state is walkable rather than theoretical.
- [ ] 1.9 Both themes; the owner's surface at phone width, the counter's mark at tablet width.
- [ ] 1.10 SECTION GATE — at `/demo`: the owner searches, opens the card, grants, confirms, reopens, revokes, re-grants and renames; a biller identifies that customer and sees the mark on every one of the five surfaces.

## 2. 🧍 OWNER CHECKPOINT — STOP HERE

- [ ] 2.1 Hand the owner the demo and let them walk it. Expect several rounds; iterate on 1.2–1.9 as they ask.
- [ ] 2.2 Record what changed between `design.md`'s sketch and what the owner settled on, including anything about the mark's placement that #56's final shape forced.
...
- [ ] 3.6 Owner lookup by complete phone. `customer_directory()` returns the entire directory ordered by `created_at`, unpaginated — written when the table had zero rows — so it needs a phone argument and a bounded recent list. Prefix and name search are *permissible* at the owner's boundary and are deliberately **not** built for v1.
...
- [ ] 3.9 Refusal cases for every path added here: an FA, a Biller, an Employee and a counter device each refused grant, revoke, rename and the figures, **by hand-crafted request carrying a valid session**, not by an absent button.
- [ ] 3.10 Regenerate schema types and commit the diff.
- [ ] 3.11 SECTION GATE — grants, revocations, re-grants and renames round-trip at the database; the figures agree with the bills they derive from; and every one of the four non-owner principals is refused every owner path.
...
- [ ] 4.3 Widen `customer_lookup_by_phone` by exactly one field — current membership as a plain yes or no. **No date, no actor, no history, no spend, no visit, no outlet.** Nothing else about that boundary moves: not the exactness, not the absence of a browse path, not the rate bound, not the attempt table's rule against recording what was asked.
