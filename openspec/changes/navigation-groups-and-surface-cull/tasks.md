...
- [x] 3.6 Seed the open group from the address, re-seed it ~~only when the reader crosses into or out of a group~~ **on every move**, and ~~refuse to close a group the reader is standing inside~~ — **both narrower rules were dropped by the owner on 2026-09-01**; see `design.md`. Every group toggles, and a hand-opened card collapses on going anywhere outside it, including the Attendance-to-Overview move that crossing no group left it standing over.
