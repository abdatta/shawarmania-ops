...
**GPT-5.6 Sol** drives **#23** — an owner decision of 2026-07-30, and the first change driven by a non-Claude agent. Its seed is written fully self-contained for exactly that reason: the driver arrives with none of this project's context, so the proposal carries the orientation (people model, privileged path, adapter seam, verification commands) that a returning session would already hold. The repo's Codex skills (ported 2026-07-30) give it the same `/opsx` workflow.
...
- **Wave D — the counter takes money (#21, #22, #23, #9, #10)**: `staff-as-accounts`, then `multi-outlet-people`, then `multi-outlet-hiring`, then `counter-devices-and-offline`, then `billing-live`. **#21 opens the wave**: it collapses the staff roster into accounts (owner decisions of 2026-07-28, recorded in its proposal), and #9's shift attribution must be designed against the merged people model rather than a table scheduled to die. It also lands while production staff data is still at baseline — the cheapest this migration will ever be. **#22 follows it directly** (graduated from `openspec/todos/` on 2026-07-29, its trigger having fired on both counts at once — a staffer splitting shifts across the outlets, and the owner day-running one): a person may be **assigned** to more than one outlet, with the database deciding every write by membership rather than by anything session-scoped — no role switching, nothing token-borne, nothing new for staff to learn (the owner rejected the earlier grants-and-active-hat sketch as needless complexity, 2026-07-29). Staff codes retire in the same change, and the owner's non-cash remote entries fold in by the same decision. It sits before #9 for the same two reasons #21 opened the wave: shift attribution should be designed against the assignments model, and the roles-model migration is cheapest while production still holds two accounts. #9 before #10: billing is offline-first by specification, so building settlement online-first and retrofitting a queue would mean rewriting the path the whole product depends on. (#9's former dependency on #5 was the attendance kiosk, which the owner rejected on 2026-07-28 — the fallback for a phone that cannot check in is manager-entered attendance, built in #21.) **#23 slots between #22 and #9** (seeded 2026-07-30 straight from an owner decision, skipping `todos/` because its trigger had already fired in conversation): #22 left hiring at one outlet per action, and the create-then-assign workaround silently kills the activation code just issued — an onboarding trap live in production, cheapest to fix while the #22 surfaces are fresh and before more staff are onboarded. It touches nothing #9 depends on, so the two may also run in parallel.
