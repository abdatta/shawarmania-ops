...
**Opus drives #9, #33, #10, #34, #35, #41, #11, #12, #51 and #14.** These require
...
(#41), the drawer as a running balance (#11), retiring the stopgap ledger (#12),
two-level navigation and the surface cull (#51), and end-to-end third-outlet
onboarding (#14).
...
graph TD
    ...
    C12 --> C51[51 navigation-groups-and-surface-cull]
    C51 --> C14[14 outlet-onboarding]
```
...
- **Wave E — operations go live, and insight is withdrawn (#11, #12, #51)**: `cash-is-counted-not-closed`, `retire-the-manual-ledger`, `navigation-groups-and-surface-cull`. **#11 is the payoff of the whole billing chain**, the surface that answers "is the drawer right?", and it needs real bills to mean anything, which is its only dependency.

  **#13 `owner-console-live` was withdrawn on 2026-08-31, and #51 is what replaced it.** It was the change that would have made the P&L, the two-outlet comparison and the period reports real. The owner reviewed the app against the business as it actually runs and concluded those three answer questions nobody at Shawarmania asks: the Ledger already says what a day took and what it cost, from recorded rows, and an estimate on top of it is a second number to reconcile rather than an answer. So the wave that was going to *promote* those screens now *deletes* them, along with the alert thread and the stock ledger, and spends the room on the navigation the sixteen surviving surfaces had outgrown. Its reasoning is preserved in [`openspec/todos/owner-console-was-withdrawn.md`](../todos/owner-console-was-withdrawn.md) rather than discarded, because "the owner never asked for a P&L" is a fact about August 2026 and not a law. **The tables stay**: #51 deletes screens and touches no migration, so the decision is reversible in the way that matters.
...
- **[`customer-loyalty-and-cross-outlet-insights`](../todos/customer-loyalty-and-cross-outlet-insights.md)** — #32 provides one safe global identity but deliberately no cross-outlet history, visit/spend aggregate, marketing, or reward behavior. **Trigger**: a concrete loyalty or repeat-customer decision needs activity across outlets, with its audience and franchise/privacy boundary decided first. Deps when seeded: #32 and #10. (It named #13 as a likely third until that change was withdrawn.)
