...
| 🔄 | 40 | D | `account-lifecycle-truth-and-safe-transitions` | **GPT-5.6 Sol (high lead)** | active | #10, #22, #24, #30 | an admin edits a person's facts and permitted outlet roles through one truthful workflow; promotion, transfer, reset, activation, username correction, deactivation, and invalid-session recovery each do exactly what their label says without accidentally ending access; FA/SA/self/final-owner boundaries are proved by hand-crafted requests; and offline uncertainty never destroys a valid session |
