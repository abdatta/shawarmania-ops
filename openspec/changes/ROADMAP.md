...
| 📝 | 24 | D | `username-sign-in-and-owner-recovery` | **GPT-5.6 Sol** | proposed | **#23** | **an admin creates a person without an email; the person opens one activation link, types the username shown there and matching new passwords, Chrome-compatible semantics can save that username/password pair, and the person signs in with it**; only a Super Admin carries recovery email and can recover without another admin; every existing account, assignment, password, session, invite, attendance row, and tenancy boundary survives the move |
|  | 9 | D | `counter-devices-and-offline` | **Fable** | seeded | #4, #21, #22, **#24** | an enrolled tablet reaches only its own outlet and revoke is immediate; offline → 20 bills → online → exactly 20 rows, zero duplicates; queue survives restart; a replayed UUID inserts once |
...
| App accounts, usernames and one-time links | everything | #4 → **#24 replaces staff email sign-in** |
| Super Admin recovery contact | owner self-recovery | **#24**, private and owner-only |
...
- **[`self-service-account-settings`](../todos/self-service-account-settings.md)** — a signed-in person can request a username change or change a password they still know from a shared Profile/Settings surface. Forgotten-password recovery stays role-specific: Super Admin uses the private recovery channel from #24, while every other role asks an authorized admin for a one-time link. **Trigger**: the shared Profile/Settings surface is built or the first real request arrives. Deps: #24; Biller behavior must be rechecked after #9.
