...
| 📝 | 31 | D | `ui-billing-lifecycle` | **GPT-5.6 Sol** | proposed | #6, #7, #9, #32, #33 | the counter takes immediate payment with its guaranteed Undo, or saves an order with a number that is called out, reopens, edits, pays and cancels it, including an aggregator order collected by a rider; exact-phone autofill, shift history, originating-tablet exception handling, a manager's void followed by a manual counter re-ring, and a manager clearing a stranded order are all walkable in demo mode without touching Supabase or offering discounts |
