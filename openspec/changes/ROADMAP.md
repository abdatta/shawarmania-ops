...
| 📝 | 40 | D | `account-lifecycle-truth-and-safe-transitions` | **GPT-5.6 Sol (high lead)** | proposed | #10, #22, #24, #30 | an admin edits a person's facts and permitted outlet roles through one truthful workflow; promotion, transfer, reset, activation, username correction, deactivation, and invalid-session recovery each do exactly what their label says without accidentally ending access; FA/SA/self/final-owner boundaries are proved by hand-crafted requests; and offline uncertainty never destroys a valid session |
...
|  | 39 | E | `counter-seen-and-practised` | **Opus** | seeded | #10 | **from their own phone, away from the outlet, the owner opens a counter's own screen and sees what the biller sees** — the same components the tablet renders, read-only, stated as of one reading, with every write refused by the database and proved by a hand-crafted request; a Franchise Admin reaches it for outlets they manage and no others; the owner alone takes that reading into an editable **practice copy** carrying the outlet's real data under its own banner, reachable by no link, writing nothing anywhere and dying with the tab; `/demo` stays fabricated, public and isolated, and the four-role demo walkthrough still walks |
...
graph TD
    ...
    C11 --> C13
    C12 --> C13
    C10 --> C39[39 counter-seen-and-practised]
...
- **Wave E — operations and insight go live (#11–#13)**: `expenses-and-inventory-live`, `daily-cash-live`, `owner-console-live`. **#12 is the payoff of the whole billing chain** — the screen that answers "is the drawer right?" — and needs both real bills and real cash expenses to mean anything. #11 depends only on #4 and #7, so it **may start alongside Wave D**.
  ...
  **#39 is in this wave by dependency alone**: it needs #10's real counter and nothing else, blocks nothing, and may run at any point after Billing V1. It comes from the owner's Kalyani session on 2026-08-12 — the wish to open a counter's own screen from home, and to poke at a copy of it safely. **The readable half of that wish is already #10's**, which grew the Tablets card into the live shift and its figures; #39 is the part that mounts the biller's actual workspace somewhere else, and it is separate because it is a refactor of the counter under a counter that is by then taking real money. It also closes the drift the owner found: the demo's billing shell and the tablet's are two compositions of the same columns, which is why one has Finish day and the other never did. A third hand-built counter view would drift the same way, so #39 builds none — it mounts the one that exists, three ways.
