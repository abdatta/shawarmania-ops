...
**Reason**: Check-out is removed from the product, and a check-in no longer
stands on its own: it is a claim that a recorded approval settles. Replaced by
"An employee checks in from their own phone, and the day waits for a manager".

**Migration**: Every recorded check-out time and location is dropped from the
database by this change, with a production dump taken beforehand and held
outside the repo. No client, surface or spec refers to a check-out afterwards.

### Requirement: A closed outlet accepts no new check-ins and never blocks a check-out

**Reason**: The check-out half of this requirement no longer has a subject.
Replaced by "A closed outlet accepts no new check-ins", which keeps the
check-in refusal verbatim and restates the asymmetry as approval never being
refused for that reason.

**Migration**: None. The check-in behaviour is unchanged.

### Requirement: An override is a recorded human decision

**Reason**: An override was the exception path for a check-in the fence
refused. Approval is now the only path for every check-in, so the word
describes nothing, and the rule that governs it is no longer about who the
approver is but about whether the fence vouched for them. Replaced by "Only a
recorded approval settles a day" and "An approval records where the approver
was, and being elsewhere costs a reason".

**Migration**: The stored columns are renamed from `override_*` to the
approval they now are. Every historic override carried an approver, a time and
a reason, which is exactly an off-site approval under the new rule, so no
existing row changes meaning and none is recomputed.

### Requirement: Location is captured only at check-in and check-out

**Reason**: Check-out no longer exists, and approval is a new moment at which
a position is read. Replaced by "Location is captured only at check-in, at
approval, and at an outlet capture", which extends the same duty to the
approving manager's device.
