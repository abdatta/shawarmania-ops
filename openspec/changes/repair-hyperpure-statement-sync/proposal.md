...
not exist, so neither the reader nor the manual upload fallback can finish.
...
- Make the shared statement parser select enabled restaurant mappings through
  their actual `state` field.
