...
- [ ] 1.1 Add the unproven-setup state: a redeemed code creates a row that is not an active tablet, appears nowhere, reaches nothing, and expires on its own — with the singleton index still in place, so the two halves of the invariant are never both loose at the same time.
...
- [ ] 2.6 Say plainly on the outlet pipeline which tablet took an order, so an operator understands a refusal before they meet it.
...
- [ ] 4.1 Drive two independent browser contexts as two tablets at one outlet through concurrent online payment, one tablet going offline and capturing while the other trades, interleaved reconnect, a lost response, an attempted action on the neighbour's order, and the removal of one tablet mid-service.
