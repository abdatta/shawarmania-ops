...
- [ ] 1.1 `CustomerDialog`: a `Modal` with a numeric readout and an eleven-key pad (`1`–`9`, `00`, `0`, backspace) gridded like `PaymentDialog`, no search control, and no system keyboard needed for the number.
- [ ] 1.2 Resolution on the tenth digit and not before: below ten digits the resolution area renders nothing at all — not a spinner, not a hint, not a greyed card. Drive it from `shared/phone`'s existing canonicalisation, adding no second phone rule.
- [ ] 1.3 The match state: saved name shown, action reads **Use**. The no-match state: one optional name field, action reads **Save**. A ten-digit number the Indian mobile rule still refuses reads as no match **and offers no save**, since a number that cannot be stored must not present a form that fails on submit.
- [ ] 1.4 Skip: swaps the dialog body to a single label field and a Done, without closing and reopening. No reason prompt, no approval, no limit. **Reachable only from inside the dialog** — adding it to the composer is the failure mode this change exists to prevent.
- [ ] 1.5 The composer's customer row in three states — nothing chosen, chosen (`Rahul · +91 98765 43210`), skipped (`Rahul (label only)`) — with the clear action returning it to nothing chosen. Leave the slot where #57's ⭐ will sit and draw nothing in it.
- [ ] 1.6 Delete the two side-by-side inputs and the *"Add a customer name or phone to continue"* line from `bill-composer-footer.tsx`. Replace `hasCustomerIdentity` with the decision gate: identified **or** skipped enables Order and Mark Paid.
- [ ] 1.7 Wire it to the **mock** customers adapter only, and make the demo fixtures carry a customer with a saved name, one with none, and a number that matches nobody, so all three states are walkable at `/demo` without a backend.
- [ ] 1.8 Both themes and a tablet viewport, one-handed: the pad is for a thumb and the dialog must not need a scroll to reach Skip or the action.
- [ ] 1.9 SECTION GATE — at `/demo` as a Biller, every state walks on the mock: match, no-match-then-save, skip-with-label, skip-with-nothing, clear-and-rechoose, and nine digits showing nothing.
