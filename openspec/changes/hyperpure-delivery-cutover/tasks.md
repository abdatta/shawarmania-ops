...
- [ ] 1.1 Add `supplier_delivery_routes` with `source_system`, explicit
...
- [ ] 1.2 Seed Hyperpure's historical-default route from `0001-01-01` to
...
- [ ] 3.2 Change Hyperpure ingest conflict handling to the global key; update the
  one row's mutable statement facts but never silently change its outlet.
- [ ] 3.3 Refuse an existing Hyperpure identity whose stored outlet disagrees
...
- [ ] 4.3 Move the snapshotted set to Kalyani through a transaction-bounded
  migration-only bypass of the expense identity trigger; restore the trigger
  before commit.
- [ ] 4.4 Assert afterwards that every snapshotted row retains its id, source
...
- [ ] 4.5 Add a migration rehearsal fixture matching the production boundary;
...
- [ ] 5.1 In `abdatta/shawarmania-sync`, update documentation so
...
- [ ] 5.3 Leave `HYPERPURE_DELIVERY_OUTLET_ID=1719650`, the Zomato picker card,
  stored credentials, capture workflow, login workflow and OTP mailbox code
  byte-for-byte unchanged.
- [ ] 5.4 Add or retain source-contract tests proving an ordinary Hyperpure read
...
- [ ] 6.1 Update `docs/DATA_MODEL.md` with dated supplier routes, global
  Hyperpure identity, the attribution-conflict rule and the compatibility marker.
- [ ] 6.2 Update `docs/OPERATIONS.md` with the cutover procedure, safe deployment
  order, no-OTP verification boundary and run-health variable.
- [ ] 6.3 Update `docs/BUSINESS_CONTEXT.md`: physical Hyperpure deliveries are
...
- [ ] 7.1 Before code changes, capture the production baseline by source ref,
...
- [ ] 7.3 Because this changes a table, RLS, migration and money attribution,
  ...
  generated-type diff.
- [ ] 7.4 Run the companion sync repo's Hyperpure, session, workflow, format and
