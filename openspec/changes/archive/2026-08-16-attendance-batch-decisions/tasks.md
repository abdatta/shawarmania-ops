...
## 9. Selection entered from the row (design D10)

Everything section 4 and 5 proved stays proved. This section moves where
selection is entered, what carries the set's actions, and what a waiting row can
be collapsed to. No database, adapter, policy or command behaviour changes.

- [x] 9.1 Give `AttendanceCard` a `pinnedOpen` prop that forces the panel open and disables the header toggle while still rendering the chevron, so nothing shifts when a row settles and the chevron becomes live. Leave `day-range-list.tsx` passing nothing, so the by-staff month keeps its chevron. **The inert chevron is dimmed rather than hidden**, and measured in the browser: a waiting row reads `disabled` with `aria-expanded="true"`, a settled one `false`/`false`.
- [x] 9.2 Remove the `aside` prop and the `Select people to decide together` toggle row, and drop `selecting`, `canSelect` and `onToggleSelecting` from `DayPicker`.
- [x] 9.3 Replace the selecting flag with the set itself: selection is on exactly when at least one row is chosen, so the first press enters it and `Clear`, the last deselection, a refusal dropping every row and a reload that settles every selected row all leave it. **Derived from `chosen`, not from `selected`**, so a row settled by somebody else drops the mode as well as the count.
- [x] 9.4 Put the selection control in the row's action group, leftmost, ahead of `Approve` and `Deny`: an unlabelled empty or checked box, disabled while an action is saving, with an accessible name and title naming the person. **Square rather than the sized variant's default**, so the box sits the same distance from every edge, outlined like `Deny` when empty and filled like `Approve` when checked, so who is in the set is legible down a scrolled roll-call.
- [x] 9.5 Hide the per-row `Approve` and `Deny` on every waiting row while a set exists, and leave `Record arrival` and `Correct attendance` live throughout.
- [x] 9.6 Put the day picker and the selection bar in one sticky slot at the top of the scroll, the bar replacing the picker while a set exists, both the same height, the bar holding one line at 375px. **Measured in the browser at 375px:** both 54px tall, the roll-call's top at 293px in both states, all three bar buttons on one row, and the slot pinned at the viewport top under scroll.
- [x] 9.7 Confirm by route rather than by count: everything decided from the bar is named back before the write, including a set of one; the per-row buttons confirm nothing. Fix the sheet titles that read `1 arrivals` and `1 check-ins`. **The reason sheet's footer follows the route too**, so a set of one from the bar reads `Continue` rather than offering to write immediately.
- [x] 9.8 Rework the component suite for the new entry: no `toggle-selecting`, selection entered by pressing a row's box, the mode leaving by itself at zero, a set of one confirming from the bar and not from the row, and the banned-control sweep unchanged. **143 attendance component assertions pass.**
- [x] 9.9 Rework `e2e/attendance.spec.ts` the same way, including that a waiting row's chevron is present and disabled and that its body cannot be collapsed. **240 Playwright specs pass**, two more than before this section.
- [x] 9.10 Update `docs/SCREENS.md` for the row-entered selection, the replaced picker and the pinned waiting row. **`DEMO_MODE.md`, `OPERATIONS.md` and `TESTING.md` also described the toggle row** and are corrected in the same pass.
- [x] 9.11 Make the confirmation the whole final state rather than a list of names: the reason quoted back in the manager's own words, an approval's one position reading stated in the terms the row will carry afterwards, and a denial's retry choice stated whichever way it was left.
- [x] 9.12 Toggle a row from the previous state rather than from the render's closure, so two presses arriving in one React batch cannot silently drop one of the two people. **Found by hand in the browser and proved both ways**: the new test fails with `1 selected` against the closure form and passes against the updater form.

## 10. Full gate

- [x] 10.1 Run `npm run lint`, `npm run format:check`, `npm run typecheck`, `npm test`, `npm run test:db`, `npm run test:rls`, `npm run contrast`, `npm run build`, and `npm run test:e2e` in both themes at phone and tablet widths, plus `npm run test:e2e:auth`. **Re-run in full after section 9**: 1262 unit and component tests, 1425 database assertions, the three REST/RLS configs, 50 contrast pairs in both themes, 240 Playwright specs and the 21 real-backend auth specs. The same gate passed again on CI against the pushed commit.
- [x] 10.2 Run `npm run roadmap:sync` and confirm the board reflects this change's real state.
- [x] 10.3 PHASE GATE, proved by the suites in sections 1, 2, 6 and 8 rather than by hand: an FA or SA adds each waiting employee by one manual action, with no Select all and no subset shortcut, then approves or denies the explicit set atomically after confirming the named people; one fresh position is judged independently against each selected row's own outlet and date, a common reason reaches only approvals that require it, denial reads no manager position and applies one stated retry choice to all, stale or unauthorised state changes none, every person retains an immutable decision carrying a shared batch identity, and the four-role demo walkthrough still walks.
