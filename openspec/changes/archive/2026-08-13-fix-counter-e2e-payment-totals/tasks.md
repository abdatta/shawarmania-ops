...
- [x] 1.3 Record the manager-only payment-total and non-financial tablet-status
  contracts as delta specs for the living requirements.

## 2. Verification and phase gate

- [x] 2.1 Run `npm run typecheck` and the affected E2E test file.
- [x] 2.2 PHASE GATE — the production Deploy workflow's browser gate passes while payment totals remain only in the manager billing-history Totals view.
