...
- [x] 0.1 Read `design.md` end to end. Its D1 carries the exact ceiling
...
- [x] 0.2 Read the deleted screen and its arithmetic, which are the design
...
- [x] 0.3 Confirm `#51 navigation-groups-and-surface-cull` has archived. This
...
- [x] 1.1 Widen `LedgerStatementMonth` in `src/data-access/adapters.ts` with a
      ...
      throughout.
- [x] 1.2 Accumulate channels into a **map keyed by `channel`**, not two named
      fields (`design.md` D2). Render order is stable; a third channel needs no
      change here.
- [x] 1.3 Implement the ceiling arithmetic exactly as `design.md` D1 states:
      `net += day.netPaise ?? day.grossPaise`, and commission **derived** as
      `gross - net`. Do not accumulate commission independently.
- [x] 1.4 Count a day as undetermined when **any** of its channels is, and carry
      the count, not a boolean.
- [x] 1.5 Carry each channel's `asOfAt` as the latest of its days, comparing the
      ISO-8601 strings lexicographically rather than constructing a `Date`.
- [x] 1.6 Add `category` to the expense row shape as its own field, leaving
      `label` untouched (`design.md` D6). `effective_expenses` already exposes
      the column; no migration and no view change.
- [x] 1.7 Do all of the above in **both** adapters —
...
- [x] 1.8 Reduce the drawer half of the month reading to the counted / carried /
      not-tracked-yet tallies. The thirty-one-row array stops being returned.

## 2. Dates with no bills

- [x] 2.1 Derive the unbilled dates from the thirty-one day readings the month
...
- [x] 2.2 Carry both the **count** and the **dates themselves** on the month
      reading. The surface names them; a count alone cannot.
- [x] 2.3 Mirror it in the mock adapter, and seed the demo outlets so both a
      partly-unbilled month and a wholly-unbilled one are reachable in a demo.
- [x] 2.4 Claim no cause. The reading says no sales were recorded for those dates
      and never why — before-billing, a closure and a broken tablet are
      indistinguishable to the app.
- [x] 2.5 Expenses and the drawer tallies are reported for every period,
      including one with no billed date at all.

## 3. The surface

- [x] 3.1 Rebuild `MonthReading` in
...
- [x] 3.2 Sales breakdown: Cash, UPI, then a block per channel with gross,
...
- [x] 3.3 Carry the sentence naming how many days are still waiting and why they
      settle by themselves. It is what stops a ceiling being read as final.
- [x] 3.4 Expenses breakdown grouped by category, every line beneath its category
...
- [x] 3.5 Estimated profit with `cash basis operating estimate` named beneath the
...
- [x] 3.6 Replace the thirty-one-row drawer list with the single line from
...
- [x] 3.7 Leave the **Cash out, not in operating costs** card exactly as it is,
      still outside the profit figure.
- [x] 3.8 Render the unbilled-dates note from `design.md` D3: *"N dates had no
...
- [x] 3.9 Where no date in the month carries a bill, state that there are no
...
- [x] 3.10 Drop the two lines `design.md` D7 retires: *"N days recorded"* becomes
      **days with sales**, and the *"Nothing is recorded for this month"* empty
      state goes, being unreachable.
- [x] 3.11 Update `LoadingFigures` to reserve the new shape — three cards land,
      not two.
- [x] 3.12 No role branch. The Franchise Admin reads the same component
      (`src/gates/registry.ts`), scoped by their assignments as every other
      outlet-scoped surface is.

## 4. Tests

- [x] 4.1 Arithmetic, as unit tests over the accumulation: a mid-month rate
      ...
      accepting it.
- [x] 4.2 The undetermined-day **count**, not just the flag: one waiting day and
      three waiting days produce different sentences.
- [x] 4.3 Surface tests in `src/features/cash/ledger-statement-surface.test.tsx`
...
- [x] 4.4 **A partly-unbilled month gets its own test**, seeded with the first
...
- [x] 4.5 A month with no billed date offers no profit figure at all, still lists
      its expenses, and is distinguishable in the DOM from a ceiling.
- [x] 4.6 No rendered string asserts a **cause** for an unbilled date. Assert the
      absence, the way #11 asserts that no nearby instant is proposed.
- [x] 4.7 Integer paise: a non-integer input throws rather than rounding.
- [x] 4.8 Extend `e2e/expenses-and-ledger-reach.spec.ts` so the month view is
      reached and its profit figure asserted for both the owner and the Franchise
      Admin.
- [x] 4.9 Confirm `supabase/tests/rest/zz-ledger-month-timing.test.ts` still
...
- [x] 5.1 The two deltas in `specs/` are written. Re-add `profit-estimates` to
      `openspec/specs/README.md` when it merges, or `lint:specs` fails on a
      capability with no index link.
- [x] 5.2 `docs/SCREENS.md` — the Ledger section describes the day in detail and
      the month barely at all. Add the month: three cards, the ceiling, the
      before-billing sentence, and the one-line drawer summary.
- [x] 5.3 `docs/GLOSSARY.md` — cash-basis operating estimate, and ceiling, if
      #51 removed either.
- [x] 5.4 `docs/LIMITATIONS.md` — the app produces no consumption-basis figure and
      ...
      gap to close.
- [x] 5.5 Update `openspec/todos/owner-console-was-withdrawn.md`: its trigger
      fired, and the P&L half of it is answered here. The comparison and the
      period reports remain withdrawn.
- [x] 5.6 Add the ROADMAP row (#52, wave E, depends on #51) and run
...
- [x] 6.1 `npm run lint`, `npm run format:check`, `npm run typecheck`,
      `npm run functions:typecheck`, `npm test`, `npm run contrast`,
      `npm run build`, `npm run test:e2e`.
- [x] 6.2 The Docker job: `npm run db:start && npm run db:reset`, then
...
- [x] 6.3 **A UI change**: run the app and look at it, phone and tablet
      viewports, both themes.
- [x] 6.4 **Verify the ceiling path as the normal rendering**, not the edge
...
- [x] 6.5 **Done, against production, SELECTs only.** August 2026 read at both
      outlets through the pooler and folded by the shipped `readMonth`, then
      cross-checked against independent SQL aggregates over the same rows. Every
      figure agreed at both outlets.

      Kalyani: cash ₹25,760, UPI ₹93,730, Zomato ₹50,259.07 gross over 29 dates,
      Swiggy ₹36,076 gross over 31, revenue ₹1,83,031.56 as a ceiling with 9
      dates still waiting, expenses ₹1,58,684, profit ₹24,347.56, no silent date.
      Kanchrapara: cash ₹32,140, UPI ₹61,680, Zomato ₹75,740.80 over 29 dates,
      Swiggy reporting ₹0 on 19 dates, revenue ₹1,44,622, expenses ₹2,19,693.65,
      profit −₹75,071.65, 2 silent dates.

      **Two things it caught that no local test could.** A silent date is one with
      no revenue from ANY source, not one with no bill: Kalyani was not billing
      until 12 August and still has no silent date, because the aggregators
      recorded revenue throughout — the code was right and the prose was wrong,
      and the prose is fixed (`design.md` D9). And Kanchrapara's Swiggy reports
      ₹0 on nineteen dates rather than reporting nothing, which is why a channel
      that reported nought and a channel that reported nothing must render
      differently (`design.md` D8).

      **A question for the owner, not a defect**: is Kanchrapara genuinely not
      trading on Swiggy, or is Swiggy writing zeroes there?
- [x] 6.6 The four-role demo walkthrough still walks.
