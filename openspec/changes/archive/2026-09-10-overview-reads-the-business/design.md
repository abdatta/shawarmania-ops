...
6. Reuse visible navigation definitions and attention sources for one row per destination, deduplicated by source. Summary wording is metadata with a generic fallback, so future badged pages require no Overview edit. Delivery adds one per blocked integration and preserves distinct reconciliation/duplicate work; no run-history multiplication. Hyperpure is part of the Zomato family in the shared breakdown, so top-level totals, the Zomato segment and every outlet view that exposes the account-level repair all lead to the same work. Account-level issues may repeat across outlet scopes for discoverability but are deduplicated once in navigation and Overview; scoped badges are therefore not separately additive in that state. Zero rows disappear; unresolved reads do not assert all-clear.
   A stored provider-session expiry is advisory and does not block an integration by itself: only a missing session, an OTP wait, a timed-out run, or a recorded lapse/shape failure creates intervention work. The reader deliberately tries an elapsed session because the provider remains authoritative about whether it still works.
