...
- [x] 1.0 Migration: commission stops being a rate anywhere [owner, 2026-08-17]. Convert `zomato_commission_bp` and `swiggy_commission_bp` to exact-paise amounts and drop them, rewriting existing production rows with the identical rounding `commissionPaise` always applied so no historical figure moves. Bound each commission by its own channel's revenue rather than by a percentage ceiling. Cross-check the SQL against the retired application rule over a grid of revenue and rate pairs before it touches production.
- [x] 1.1 Migration: `manual_ledger_days` holds ONE revenue and ONE commission per channel whatever the source, plus a settlement state (`provisional | settled | disputed`, null when typed) as the only marker of which source wrote them; the superseded typed pair with the moment it was superseded; and the provisional pair a settled day replaced. Net is computed, never stored: with commission exact, a third column could disagree with the two it came from. The four constraints that existed only to stop a measured triple contradicting a typed pair are deleted rather than rewritten.
...
- [x] 5.3 Present a synced day's effective rate as a computed reading only; prove no stored percentage participates in computing a synced day's net.
- [x] 5.4 Prove commission is still never an expense and never a category, as `manual-ledger` already requires.
- [x] 5.5 Integer paise throughout; no float is constructed on any path added by this change.
...
> **6.1 to 6.4 moved out of this change on 2026-08-18.** They specified how a
> *sourced* channel's group is presented while a *typed* one keeps its fields.
> `freeze-aggregator-and-supply-entry` withdraws the entry fields for every
> aggregator and moves the figures out of the day row, so building the
> two-kinds-of-group form here would be building it to delete it. The
> requirement they served has been removed from this change's `manual-ledger`
> delta and belongs to that change instead. The day form is unchanged by this
> change: it still offers typed aggregator entry, exactly as before.
- [x] 6.5 Zomato sync surface, owner-only, taking the collapsed-row shape the attendance screen already uses: a header line stating when the sync last ran and whether it succeeded, then a list of events. Runs that changed nothing produce no row.
- [x] 6.6 Event rows: a day written, a week settled, a day revised, a week disputed, a session lapsed, a possible duplicate expense. Each collapsed row states in one line what changed, and where a figure was replaced, states what it changed from as well as to.
- [x] 6.7 Rows needing the owner are presented expanded with their actions; ordinary rows stay collapsed. Prove both in one list.
- [x] 6.8 A disputed week's row shows the outlet, the cycle, both totals and the difference, and offers exactly Re-check and Accept the difference. No action writes the cycle without reconciling it or recording the difference.
- [x] 6.9 Accepting asks for confirmation naming the amount that will be recorded as unexplained, then calls the accept path from 3.4b.
- [x] 6.10 A lapsed session reads as an action the owner can take rather than a generic failure, and offers reconnecting from this surface.
- [x] 6.11 Possible-duplicate signal on this surface: a hand-entered expense sitting near a synced one is shown as a possible duplicate. Never delete either row automatically. Match on the same outlet exactly, an amount within the larger of 2% or ₹50, and a date within four days either way; do not match on category or description, which name the same purchase differently by construction. Neither row already voided or already settled.
- [x] 6.11a The row shows **both** rows in full, each with its own amount, date and note. A typed figure is rounded and dated to when the bill was noticed; reporting one amount and one date would describe one row and call it both.
- [x] 6.11b Test the tolerances against the case that actually happens: ₹3,750 typed on the 15th against ₹3,747.77 dated the 16th is one flag. A test on two identical rows would pass while catching nothing real.
- [x] 6.11c "Not a duplicate" settles the flag without touching either row, because buying the same thing twice in a day is ordinary and the flag must be answerable rather than only obeyable.
- [x] 6.12 The surface is reachable by a Super Admin only, and no other role can open it or read its rows.
- [x] 6.13 Accessible names on any field or control this change adds or relabels identify the channel, the unit and the action unambiguously; no added field's font size crosses the mobile-zoom threshold; every action on this surface is reachable and operable from the keyboard.
...
- [x] 6b.1 Edge Function `request-aggregator-sync`, JWT-verified and owner-only, re-deriving the caller's authority from their own token. It dispatches the reader's GitHub workflow with a token held as a server-side secret; no GitHub credential reaches the browser.
- [x] 6b.2 Rate-limit it, and refuse a second dispatch while a run for that outlet is still open, so a repeatedly tapped button cannot start six overlapping readers.
- [x] 6b.3 "Sync now" on the sync surface: starts a run, shows it as running, and resolves to the run's own outcome row rather than to whatever the dispatch call returned.
- [x] 6b.4 New run outcome `awaiting_one_time_password`, distinct from `session_lapsed`: the session is being repaired and the job is waiting on a person, which is a different sentence from the session having died.
- [x] 6b.5 Migration: `aggregator_auth_requests` — outlet, channel, requested_at, expires_at, fulfilled_at, and the code, held only as long as it takes the job to collect it and never in a column any client role can read. RLS and isolation tests with the table, as every other table here has.
- [x] 6b.6 Edge Function `answer-aggregator-otp`, owner-only, accepting the code the owner reads off their phone and storing it against the open request. Never logged, never echoed back, never in a URL.
- [x] 6b.7 The reader polls for the code through its own scoped endpoint, uses it once, and the request is consumed whether or not the login succeeded.
- [x] 6b.8 On the sync surface: a lapsed session expands to Reconnect; tapping it dispatches a login run; when the job asks, the row becomes a single code field with the outlet named and a countdown to expiry.
- [x] 6b.9 A code that arrives late, twice, or for an already-fulfilled request is refused identically, and a wrong code does not consume the request's remaining life in a way that tells an attacker anything.

## 7. Demo mode and fixtures

- [x] 7.1 Demo fixtures cover a provisional day, a settled day, a revised day, a disputed week, a superseded typed figure and a possible duplicate expense, so every state and every action on the sync surface is demonstrable without live credentials.
- [x] 7.2 The four-role demo walkthrough still walks, and no demo role gains sight of settlement records it should not have.

## 8. Reader job contract (private repo)

- [x] 8.1 In `abdatta/shawarmania-sync`, build the cycle payload from the dashboard JSON for the live cycle and from the settlement workbook's `Order Level` sheet for settled cycles, converting workbook doubles to paise once at parse time.
- [x] 8.2 Join Order History on `order_id` to obtain placement timestamps; report orders that cannot be timestamped rather than guessing.
- [x] 8.3 Post cycles to the Edge Function; re-read two cycles for orders and four for deductions on every run.
...
- [x] 8.5 Restrict the job to the two trading outlets, 21917311 Kalyani and 22675834 Kanchrapara. The account's third restaurant id is a discontinued outlet and is not read.

## 9. Rollout

- [x] 9.1 Set the synced-from date for one outlet and watch one full weekly cycle settle end to end.
- [x] 9.2 Set the second outlet.
