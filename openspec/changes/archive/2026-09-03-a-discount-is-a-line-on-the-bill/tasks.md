...
- [~] 6.4 The biller's own shift totals report what that shift gave away. **DISCARDED on the owner's call, 2026-09-03 — not built and not to be built.** The hold recorded here was waiting for exactly this decision, and the answer is that a giveaway figure does not belong on the billing tablet: My shift exists so a biller can reconcile the cash in the drawer against the tender split, and what the counter gave away is a question the owner asks, not one the biller can act on. Every discount stays attributed to the shift that applied it, so the manager's bill history and the day's and month's discount sections (6.1 to 6.3) already answer it for the person who asks. Marked `[~]` rather than `[x]`, because a ticked box means built.
