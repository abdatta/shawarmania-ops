...
- [x] 9.2 **The same walk on the deployed URL** — https://abdatta.github.io/shawarmania-ops/. CI green on all three jobs and Pages deployed. Confirmed live: the public landing offers no route into the demo; `/demo/owner` renders unauthenticated with both outlets carrying their own figures, the attention line, the banner and the reset; the P&L's two bases differ on the deployed build exactly as locally (₹4,609 / ₹3,215); zero console errors; every request to the app's own origin.
