...
- [x] 9.3 Owner-facing Hyperpure health **infrastructure** landed, **Model A** (Hyperpure rides Zomato's login): ops side widened `aggregator_channel_credentials` + `aggregator_sync_runs` to the channel and taught `aggregator-reader` to hold its session (migration `20260820000000`, pgTAP `38`); the reader records each run's outcome via `reportOutcome`; the app shows a read-only Hyperpure health line beside Zomato's; the reconnect that dispatches the login was fixed (it had run the reader, never the login) and the deadlock from a stale expired code request was cleared. **DEFERRED — automated Hyperpure session not yet captured:** a login-time capture step was written (`shawarmania-sync` `src/auth.mjs`), but the Zomato→Hyperpure SSO handoff does not complete headless (it goes through the partner portal's Hyperpure outlet-picker, not a bare `hyperpure.com` visit), so the automated daily Hyperpure read has no session and the Hyperpure **reconnect button is hidden**. The **manual SOA upload is the working path** for Hyperpure figures meanwhile. Carried to roadmap change #44 `aggregator-reconnect-and-hyperpure-automation`.
...
- [x] 9.7 Zomato end-to-end verified live 2026-08-20: runs read and write daily figures (through today, after the exclusive-end-date fix), and the 503-retry cured the ~06:00 UTC failures. **Hyperpure end-to-end is DEFERRED** with 9.3 — it needs the automated session, which the SSO handoff does not yet provide; carried to roadmap change #44 `aggregator-reconnect-and-hyperpure-automation`.
