...
The deployment then completes a no-write rehearsal, backfills the intended date range from real APIs/annexures and produces a coverage report comparing every legacy outlet/date with an authoritative candidate. Authoritative rows supersede legacy rows without deleting them. Dates without source coverage retain their legacy reading, marked legacy and excluded only after replacement.
