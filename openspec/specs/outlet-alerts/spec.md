...
Guarantees that an outlet can raise an issue to the owner and track it to a conclusion: a categorised, prioritised alert with a thread of responses, a status that moves one step at a time and ends terminal, and a cross-outlet inbox only the owner can read.
