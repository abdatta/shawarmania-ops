...
Makes an outlet's cash drawer a continuous balance observed at instants, rather than a record closed once a business date. A count says that a named account saw a stated amount at a stated moment; every interval figure between two counts is computed by the database and enforced as a constraint, and each count re-anchors the balance to physical cash so that a mistake or a late correction can pollute only the one interval it sits in. Every count time is approximate and says so, the surface reports what timing could account for without ever proposing an instant, and nothing here changes a person's observation on its own.
