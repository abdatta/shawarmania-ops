# Multi Device Billing Coordination

## Purpose

What holds several counter tablets at one outlet to a single, consistent
account of the day.

The rule that makes the rest safe is that **the server is the only
coordinator**. Tablets never talk to each other, share no local storage, and
agree on nothing between themselves; every fact two of them must agree about is
resolved in one database transaction. A second coordinator would be a second
thing that can be wrong, and it would be wrong silently.

From that follow the three facts a two-till outlet needs: an order belongs to
the tablet that took it and no other tablet may act on it, bill numbers are
assigned in the order the server accepts work rather than in the order it
happened, and a business date is not ready until every tablet that worked it has
confirmed its own end of day.
