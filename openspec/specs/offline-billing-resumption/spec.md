# Offline Billing Resumption

## Purpose

Guarantees that a counter tablet whose shift was approved online can be closed,
updated, force-quit or reloaded during an outage and open again on the same
shift — from a complete resume record it wrote while it could still reach the
server — without that record ever creating authority the server did not already
grant, without any remembered figure being presented as current, and without the
business day being finishable while the backend is unreachable.
