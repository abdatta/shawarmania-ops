...
business date, a category, whether it was paid in cash, an amount in integer
paise, and a **required description** of what the money was spent on. Any number
of expense rows SHALL be permitted for one outlet and business date.

The description SHALL be free text and SHALL be mandatory, because a category
and an amount alone do not identify a purchase weeks later, and an expense
nobody can identify is not a record. It SHALL be refused when blank or
whitespace-only, by the database and not only by the form, under the same rule
every other required field in the app already follows.

A category SHALL come from the existing shared expense-category list, which this
capability reuses unchanged so that a recorded row maps directly onto the live
expense record at retirement. That list SHALL NOT contain a category for
aggregator commission, for cash banked or for an owner drawing, because each of
those is accounted for elsewhere and a category for it would double-count it.
...
blank-or-whitespace description, a zero-or-negative amount and a future business
date SHALL be refused by the database.
...
#### Scenario: Commission and cash movements have no category

- **WHEN** the category list is offered
- **THEN** it contains no entry for aggregator commission, cash banked or an owner drawing

#### Scenario: Every expense says what it was for

- **WHEN** an expense is recorded
- **THEN** a description of what the money was spent on is stored with it, and the day's expense list and the month's expenses by category both show it

#### Scenario: An expense without a description is refused

- **WHEN** an expense with a missing, blank or whitespace-only description is submitted, including by a hand-crafted request
