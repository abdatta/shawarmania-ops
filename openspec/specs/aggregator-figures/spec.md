...
A channel's measured revenue and commission for a business date, held as its own
record rather than as columns on the day a person keeps. A day nobody counted is
exactly the day whose aggregator revenue still needs to be readable, so these
figures live apart from the drawer, are written only by the ingest path, and carry
where each came from and what it replaced.
