...
function lineFor(event: AggregatorSyncEvent, channelLabel: 'Zomato' | 'Swiggy'): Line {
  switch (event.kind) {
    case 'days-written':
      return {
        tag: 'Read',
        ...
        spoken: `${event.days} days of ${channelLabel} revenue read, ${range(event.from, event.to)}`,
      }
    case 'week-settled':
      return {
        tag: 'Paid',
    ...
    case 'session-lapsed':
      return {
        tag: 'Signed out',
        tone: 'wrong',
        subject: `${channelLabel} ended the session`,
        spoken: `${channelLabel} signed us out`,
      }
    case 'shape-changed':
      return {
        tag: 'Unreadable',
        tone: 'attention',
        subject: `${channelLabel}'s reply changed`,
        spoken: `${channelLabel} changed something we do not understand`,
      }
...
interface SyncEventRowProps {
  ...
  /** The portal's own name, for the words a figure cannot say. */
  channelLabel: 'Zomato' | 'Swiggy'
  busy: boolean
...
export function SyncEventRow({
  ...
  const line = lineFor(row.event, channelLabel)
    ...
    <Card className={cn('overflow-hidden p-0', actionable && 'border-danger/50')}>
      ...
      {open && (
        <div className="space-y-3 border-t border-border px-3 py-3 text-sm text-content-muted">
          ...
          {row.event.kind === 'week-settled' && (
            <p>Matches what {channelLabel} paid, to the paisa.</p>
          )}
 ...
 * exists to prevent. Check again asks the portal once more and clears most of
 * these without anybody deciding anything; accept writes the figures and records
 * the gap as its own visible line. Nothing here makes the difference disappear.
 */
  ...
  return (
    <div className="space-y-3">
      <dl className="space-y-1">
        ...
        <Figure label={`${channelLabel} paid`} value={<Money paise={event.statedPayoutPaise} />} />
        <Figure
      ...
      */}
      <ConfirmDialog
        ...
        consequence={`Records ${formatPaise(gap)} as money we cannot explain, with your name on it. No day's figures change. ${channelLabel}'s own numbers often move after a payout, so check again first.`}
