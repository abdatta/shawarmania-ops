...
describe('readDay', () => {
  it('reads a stored row as recorded, whatever the clock says', () => {
    const reading = readDay(
      record({ status: 'leave', checkIn: null }),
      OUTLET,
      '2026-07-25',
  ...
  it('reads an unsettled arrival as waiting', () => {
    expect(readDay(record(), OUTLET, '2026-07-25', new Date('2026-07-25T09:00:00Z')).kind).toBe(
      'waiting',
    )
  })
...
describe('tallyDays', () => {
  it('counts each reading once, and lateness alongside rather than instead', () => {
    const rows: { reading: DayReading; late: boolean }[] = [
      { reading: { kind: 'recorded', record: record({ status: 'present' }) }, late: false },
      { reading: { kind: 'recorded', record: record({ status: 'present' }) }, late: true },
      { reading: { kind: 'waiting', record: record() }, late: false },
      { reading: { kind: 'absent' }, late: false },
      { reading: { kind: 'not-yet-arrived' }, late: false },
    ]
...
describe('assembleRange', () => {
  const windows = [{ startedOn: '2026-07-20', endedOn: null }]
  ...
  it('paints nothing after the assignment ended', () => {
    const rows = assembleRange({
      ...
      windows: [{ startedOn: '2026-07-20', endedOn: '2026-07-22' }],
      now,
    })
