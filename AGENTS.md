...
- **Any change**: `npm run lint`, `npm run format:check`, `npm run typecheck`, `npm run functions:typecheck`, `npm test`, `npm run contrast`, `npm run build` — then `npm run test:e2e`, which builds for production and drives it. **Run `npm run format` before the gates, not after** — it rewrites files, so formatting last means every suite you already ran described bytes that no longer exist on disk. `npm run lint` bundles seven repo-invariant checks alongside ESLint: no hex outside tokens, the behaviour-backlog index, the living-spec capability index, **`lint:functions`, which fails when an Edge Function carries no `[functions.<name>]` block** — an undeclared function silently gets `verify_jwt = true` and refuses every token-free caller at the gateway — and **`lint:totals`, which fails when the bill-identity cases in `supabase/tests/47_bill_discount_arithmetic.sql` drift from `src/domain/billing-totals-cases.json`**. That last one guards a failure nothing else can see: `total = subtotal − discount + tax + rounding` is written in the check constraints, in `billTotals()` and in `billing_validate_totals`, and a version changed in one place and not the others refuses live bills at a counter rather than turning a test red. **`lint:discount-rows`**, which holds the discount row cases to the same shared table, and **`lint:agents`**, which fails when an agent instruction file under `.claude/` or `.agents/` calls the `openspec` CLI this repo does not have, or when a workflow exists in one agent tree and not the other. The prose tier also runs both index checks and `lint:agents`, so a markdown-only archive cannot let either map drift and a prose-only edit cannot reintroduce the calls.
...
- **`test:db` needs a fresh `db:reset`, and `test:rls` is what makes that true.** Two of its five phases deliberately leave rows behind — `zz-billing-command-races` commits immutable bills, and `zz-cash-drawer-writes` commits real drawer writes because its whole claim is that the Super Admin path works over HTTP. The second cleans up after itself with the service-role key; the first does not. CI gets this right by construction (`supabase start` applies the seed, then `test:db`, then `test:rls`), so run them in that order locally too — `test:db` after `test:rls` fails in bills, attendance and shift suites that have nothing to do with the change in front of you.
- **Repeated local runs hit GoTrue's rate limiter, and it looks like a test failure.** `GOTRUE_RATE_LIMIT_TOKEN_REFRESH` is 150 per window on the local stack, and `test:rls` plus `test:e2e:auth` sign in dozens of times each. A run that suddenly reports `expected 429 to be 204` across `account-flows.test.ts` is that, not a regression: wait out the window and re-run. CI runs each suite once on a fresh stack and never sees it.
...
- **UI changes**: run the app and look at it, on a phone viewport and a tablet viewport, in both light and dark themes. **Start your own preview; never attach to a server you did not start.** The owner runs `npm run dev` on 7412 by hand, and borrowing it means reporting on a hot-reloaded working tree while claiming to have checked a production build — and killing their server when you exit. The launch config's `preview` starts on 7413, beside it. **In a worktree, run on 7414** (`previewAlt`): the main tree's preview owns 7413, and `strictPort` means evicting it is not on the menu anyway. `.claude/launch.json` is **gitignored**, so a worktree starts without one — write it there yourself with a single entry on 7414 rather than copying the main tree's. Ports live in [`ports.ts`](ports.ts); because that config is untracked, it is mirrored by hand per machine and no diff will ever catch it drifting.
